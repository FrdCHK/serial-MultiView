...
class DelayConfigWindow:
    def __init__(self, root, antenna, config, default_config=None):
        ...
        self.window.title("DELAY CONFIG")
        self.window.geometry("600x140+0+660")
        self.window.minsize(width=600, height=140)
        ...
        self.labels = ["delay_smo_half_window", "delay_kalman_factor"]
        self.entries = []
        self.error_labels = []
        ...
        for i, text in enumerate(self.labels):
            ...
            entry.insert(0, self.config.get(self.labels[i], 5))
    ...
    def validate_save(self, root):
        ...
        if valid:
            ...
            root.after(1500, lambda lb=self.save_label: hide_text(lb))

    def reset(self):
        for i, text in enumerate(self.labels):
            if text in self.config_bk:
                self.config[self.labels[i]] = self.config_bk[self.labels[i]]
            self.entries[i].delete(0, tk.END)
            self.entries[i].insert(0, self.config.get(self.labels[i], 5))
