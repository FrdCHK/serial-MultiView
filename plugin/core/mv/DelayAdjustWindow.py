...
class DelayAdjustWindow:
    def __init__(self, root, antenna, config, target, primary, target_relative_position, secondary_calibrators):
        ...
        self.window.title("DELAY ADJUST")
        self.window.geometry("1320x850+600+980")
        ...
        self.present_delay_fig = None
        self.present_delay_canvas = None
        ...
        # no IF selection (mbdelay applied to all IFs)
        ...
        flag_button.grid(row=0, column=0, padx=5, pady=5)
        ...
        unflag_button.grid(row=0, column=1, padx=5, pady=5)
        reset_button = tk.Button(self.lower_frames[2], height=2, width=10, text="reset", font=self.font,
                                 command=self.on_reset)
        reset_button.grid(row=1, column=0, columnspan=2, padx=5, pady=5)
    ...
    def on_click(self, event):
        if event.inaxes is not None:
            ...
            if event.button == 1:
                ...
                x_lim = self.present_delay_fig.axes[0].get_xlim()
                self.present_delay_fig.axes[0].set_xlim(x_lim)
                self.green_line = self.present_delay_fig.axes[0].axvline(x=x, color='g', linestyle='-')
                ...
                x_lim = self.present_delay_fig.axes[0].get_xlim()
                self.present_delay_fig.axes[0].set_xlim(x_lim)
                self.red_line = self.present_delay_fig.axes[0].axvline(x=x, color='r', linestyle='-')
                self.timerange_end = x

            if self.green_line and self.red_line:
                if self.fill:
                    self.fill.remove()
                y_lim = self.present_delay_fig.axes[0].get_ylim()
                self.fill = self.present_delay_fig.axes[0].fill_betweenx(y_lim, self.timerange_start,
                                                                         self.timerange_end, color='gray', alpha=0.15)
                self.present_delay_fig.axes[0].set_ylim(y_lim)

            self.present_delay_canvas.draw()
    ...
    def delay_plot(self):
        ...
        self.timerange_start = None
        self.timerange_end = None
        if self.present_delay_fig is not None:
            plt.close(self.present_delay_fig)

        if_ids = getattr(self.antenna, "delay_if_ids", [])
        if_id = if_ids[0] if if_ids else 0
        fig = self.antenna.plot_delay(self.target_relative_position, if_id)
        ...
        self.present_delay_fig = fig
        self.present_delay_canvas = canvas
    ...
    def save(self, delay_adj_dir, delay_mv_dir):
        self.antenna.save_delay(delay_adj_dir, delay_mv_dir)
