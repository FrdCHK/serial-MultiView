...
class FitsExport(Plugin):
    ...
    @classmethod
    def export(cls, context: Context, inname: str, inclass: str, indisk: int, inseq: int, gainuse: int, source_name: str, export_dir: str, aparm: List=[0]) -> bool:
        try:
            identifier = f"{source_name} SELFCAL MAPPING"
            ...
            fits_dir = os.path.join(export_dir, f"{source_name}_FITTP.fits")
            ...
            context.logger.error(f"Error in CalibratorFitsExport.export(): {e}")
            return False
        else:
