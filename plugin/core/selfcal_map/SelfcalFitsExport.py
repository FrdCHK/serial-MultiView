...
class SelfcalFitsExport(Plugin):
    @classmethod
    def get_description(cls) -> str:
        return "Split and export FITS file for calibrators. Plugins SelfcalFringeFitting must be run before. " \
               "Plugins required: AipsCatalog, GeneralTask, SelfcalSourceSelect, SelfcalFringeFitting. " \
               "Parameter required: indisk; optional: aparm for AIPS task SPLIT."
    
    def run(self, context: Context) -> bool:
        context.logger.info(f"Start target FITS export")
        ...
        for target in context.get_context().get("targets"):
            ...
            params_export = {"inname": target["NAME"],
                            ...
                            "cl_source": f"CLCAL(FRING({target['NAME']}))"}
            ...
            if not context.get_context()["loaded_plugins"]["FitsExport"].export(context,
                                                                                ...
                                                                                " SELFCAL MAPPING"):
                context.logger.error(f"Error in target {target['NAME']} FITS export")
                return False

        context.logger.info(f"Target FITS export finished")
        return True
