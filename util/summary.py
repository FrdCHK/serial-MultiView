...
def summary(target_dir, target_row, pr_flag=True, mv_flag=True):
    ...
    if pr_flag:
        with open(os.path.join(target_dir, pr_file_name), 'r', encoding='utf-8') as file:
            for line in file:
    ...
    if mv_flag:
        with open(os.path.join(target_dir, mv_file_name), 'r', encoding='utf-8') as file:
            for line in file:
                pattern = re.compile(r'Solution RMS\s*([+-]?\d+(\.\d+)?[eE][+-]?\d+)\s*in\d+\s*usable pixels')
                match = pattern.search(line)
                if match:
    ...
    if pr_flag or mv_flag:
        summary_file_name = os.path.join(target_dir, f"{target_row['ID']}-{target_row['NAME']}-SUMMARY.csv")
