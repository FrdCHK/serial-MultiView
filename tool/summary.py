...
def summary(exp_dir, targets):
    for i, row_i in targets.iterrows():
        pr_file_name = f"{row_i['ID']}-{row_i['NAME']}-PR.jmfit"
        mv_file_name = f"{row_i['ID']}-{row_i['NAME']}-MV.jmfit"
        summary_df = pd.DataFrame(columns=['MODE', 'PEAK', 'PEAK_ERR', 'RA', 'RA_ERR', 'DEC', 'DEC_ERR',
        ...
        with open(os.path.join(exp_dir, pr_file_name), 'r', encoding='utf-8') as file:
            for line in file:
                pattern = re.compile(r'Peak intensity\s+=\s+([+-]?\d+(\.\d+)?[eE][+-]?\d+)\s+\+/-\s+([+-]?\d+(\.\d+)?[eE][+-]?\d+)')
        ...
        summary_file_name = os.path.join(exp_dir, f"{row_i['ID']}-{row_i['NAME']}-SUMMARY.csv")
